...
def test_convert_to_backend_to_pandas():
    """Test converting DataFrame with to_pandas method."""
    # Get DataFrame with to_pandas method
    df = generate_synthetic_time_series(backend="dask", num_samples=10, num_features=2)
...
def test_convert_to_backend_invalid_input():
    """Test convert_to_backend with invalid input."""
    ...
    with pytest.raises(UnsupportedBackendError, match="Input DataFrame type 'InvalidDataFrame' is not supported"):
        convert_to_backend(InvalidDataFrame(), "pandas")


def test_convert_to_backend_to_pandas_method():
    """Test converting DataFrame with to_pandas method."""
...
def test_convert_to_backend_compute_error():
    ...
    with pytest.raises(UnsupportedBackendError, match="Unsupported backend: Failed to convert DataFrame: Computation failed"):
        convert_to_backend(df, "pandas")


def test_convert_to_backend_validation_error():
    """Test error handling when DataFrame validation fails."""

    class BrokenDataFrame:
        pass  # No conversion methods available

    df = BrokenDataFrame()

    with pytest.raises(UnsupportedBackendError, match="Input DataFrame type 'BrokenDataFrame' is not supported"):
        convert_to_backend(df, "pandas")
