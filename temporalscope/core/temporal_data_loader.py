...
class TimeFrame:
    ...
    :type sort: bool
    :param partitioner: Optional. A temporal partitioner for splitting the data. Default is None.
    :type partitioner: Optional[BaseTemporalPartitioner]
    ...
    def __init__(
        ...
        backend: str = "pl",
        sort: bool = True,
        partitioner: Optional[BaseTemporalPartitioner] = None,
    ):
        ...
        self._partitioner = partitioner  # Add the partitioner to the initialization
    ...
    def apply_partitioning(self) -> None:
        """Optionally apply partitioning using the partitioner if one is provided."""
        if self._partitioner:
            self.partitions = self._partitioner.get_partitions()
        else:
            raise ValueError("No partitioner has been provided.")

    def get_partition_indices(self) -> List[Tuple[int, int]]:
        """Return the partition indices."""
        if not self._partitioner:
            raise ValueError("No partitioner has been provided.")
        return self._partitioner.get_partitions()
